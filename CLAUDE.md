...
- **Keyword categorization uses substring matching** — `categorizeEvent()` checks if any keyword is a substring of the event text. "workout" matches the "work" keyword, causing fitness events to be categorized as work if work keywords are checked first. First area match wins based on object iteration order.
...
- **Test files are excluded from `tsc` build** via `tsconfig.json` exclude pattern, but Vitest picks them up fine.

## Spec

The full product spec is at `/Users/jamesroche/projects/ideas/personal-planner/README.md`. Phase 1 (MVP) is complete. Phase 2 (habits, energy learning, SQLite) is not started.
